...
/* 
Template Name: leaderboard-promotion
*/
    ...
    if($merchant=='stripe'){
        try{
    		...
    		$customer=Stripe\Charge::create ([
    			  ...
    			  "currency" => 'USD',
    			  "source" => $token,
    			  "description" => 'Leader Board Promotion' 
    		]);
    	...
    	if($stripeResponse=='success'){
            mysqli_query($con," INSERT INTO `leaderBoardPromotion`(`userID`, `product`, `promotion`, `duration`, `startDate`, `endDate`, `paid`, `status`) 
...
if(isset($_GET['paypalReturn'])){
    mysqli_query($con," UPDATE `leaderBoardPromotion` SET `paid`='1',`status`='1' WHERE `id`=".$_GET['paypalReturn']." ");
    header("location: /leaderboard-promotion/");
}
...
<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" id="paypal_main_form" method="post" style="display:none;">
    ...
    <input type="hidden" name="item_name" value="Leader Board Promotion" />
    <input type="hidden" name="amount" value="<?php if(isset($paypalPrice)){ echo $paypalPrice; } ?>" />
    <input type="hidden" name="currency_code" value="USD" />
    <input type="hidden" name="return" value="https://bully.cloudstandly.com/leaderboard-promotion/?paypalReturn=<?php if(isset($leaderBoardID)){ echo $leaderBoardID; } ?>">
        ...
        <div class="row">
            <div class="template-pages-content">
                ...
                <div class="template-sidebar">
                    <?php $activeSidebar="leaderboard-promotion"; include "custom-sidebar.php"; ?>
                </div>
                <!-- Start Template Page Content-->
                <div class="main-content-class">
                    <div class="inner-page-content">
                        <div class="template-page-title">
                            <h2>Leaderboard Promotion</h2>
                        </div>
                            ...
                            <form id="shopForm" class="formValidationQuery" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="flex-block flex-block04">
                                        <div class="flex-block-item flex-block-item01">
                                            ...
                                            <input type="number" id="days-weeks-months" min=1 name="DaysWeeksMonths" placeholder="1" required />
                                        </div>
                                    </div>
                ...
                <div class="history-tablel-records">
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            <?php
                                $leaderBoardPromotions=mysqli_query($con,"SELECT * FROM `leaderBoardPromotion` where `userID`='".$_COOKIE["user_id"]."' "); 
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a class="paid">Paid</a>
                                    <?php }else{ ?>
                                    <a style="cursor:pointer;" data-price="<?php if(isset($productPromotion['id'])){ echo $productPromotion['price']; }else{ echo 0; } ?>" data-id="<?php echo $leaderBoardPromotion['id']; ?>" class="unpaid paypalButtonPayNow">Pay Now</a>
                                    <?php } ?>
                                </td>
                            </tr>
                            <?php } ?>
...
jQuery(document).on("click","a.paypalButtonPayNow",function(){
    ...
    jQuery('form#paypal_main_form input[name="return"]').val('https://bully.cloudstandly.com/leaderboard-promotion/?paypalReturn='+id);
    jQuery('form#paypal_main_form input[name="submit"]').click();
});
</script>
