...
if(isset($_POST['submit'])){
    ...
    $query = " INSERT INTO `productPromotionStore`(`userID`, `product`, `promotion`, `duration`, `startDate`, `endDate`, `paid`, `status`) 
...
if(isset($_POST['update'])){
    ...
    $query = " UPDATE `productPromotionStore` SET `product`='".$product."',`promotion`='".$promotion."',`duration`='".$duration."',`startDate`='".$startDate."',`endDate`='".$endDate."',
    `paid`='".$payment."',`status`='".$status."' WHERE `id`='".$id."' ";
    $wpdb->query($query);
    
}
if(isset($_GET['productPromotionStore-delete'])){
    $query = "DELETE FROM `productPromotionStore` WHERE id=".$_GET['productPromotionStore-delete'];
    $wpdb->query($query);
    header("Location: /wp-admin/admin.php?page=bully-product-promtions");
}
?>
...
<div class="maincontent zones">
    <div class="actionButtons">
        <button type="button" data-bs-toggle="modal" data-bs-target="#vendorsDetailed">Add Product Promotion</button>
    </div>
    <table id="dataTableID" class="table table-striped table-bordered vendors" cellspacing="0" width="100%">
    	...
    	<tbody>
    	    <?php
    	            $i=1;
    	            $productPromotionStores = $wpdb->get_results("SELECT * FROM `productPromotionStore` order by id desc ");
            ...
            ?>
        		<tr>
        			...
        			<td>
        			    ...
        			    <a href="/wp-admin/admin.php?page=bully-product-promtions&productPromotionStore-delete=<?php echo $productPromotionStore->id; ?>" class="delete"><i class="fa fa-trash"></i> Delete</a>
        			</td>
        		</tr>
    		<?php
    		        $i++;
  ...
  <div class="modal-dialog">
    <div class="modal-content">
        <form method="post" enctype="multipart/form-data">
          ...
          <div class="modal-header">
            <h5 class="modal-title" id="vendorsDetailedLabel<?php echo $productPromotionStore->id; ?>">Update Promotion</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
                <p>
