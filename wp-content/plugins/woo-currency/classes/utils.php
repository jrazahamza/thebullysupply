...
class utilsWcu {
	...
	static public function escape($data) {
		return sanitize_text_field(esc_html($data));
	}
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsWcu::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsWcu::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameWcu::_()->getModule($m))
                return false;
            if(!is_dir(frameWcu::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(frameWcu::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqWcu::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worwcuess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqWcu::getVar('HTTP_REFERER', 'server');
	}
	...
	static public function isHPOS() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\OrderUtil::class ) ) {
			return \Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled();
		}
		return false;
	}
	static public function updateOrderMeta( $orderId, $metas ) {
		if (!self::isHPOS()) {
		...
		$order = frameWcu::_()->getOrder($orderId);
		if ($order) {
			foreach ($metas as $key => $value) {
	...
	static public function getOrderMeta( $orderId, $key, $flag = true ) {
		if (!self::isHPOS()) {
			return get_post_meta( $orderId, $key, $flag );
		}
		$order = frameWcu::_()->getOrder($orderId);
		if ($order) {
			if ('_order_currency' == $key) {
				$mod = frameWcu::_()->getModule('currency');
				remove_action('woocommerce_order_get_currency', array($mod, 'getOrderCurrency'), 1, 2);
				$cur = $order->get_currency();
				add_action('woocommerce_order_get_currency', array($mod, 'getOrderCurrency'), 1, 2);
				return $cur;
			}
			$metas = $order->get_meta_data();
			foreach ($metas as $metaObj) {
				$data = $metaObj->get_data();
				if ($data['key'] == $key) {
					return $data['value'];
				}
			}
			//return $order->get_meta($key, $flag);
		}
		return false;
	}
	static public function isOrderType( $id ) {
		if (self::isHPOS()) {
			return 'shop_order' === \Automattic\WooCommerce\Utilities\OrderUtil::get_order_type($id);
		}
		$post = get_post($id);
		return 'shop_order' == $post->post_type;
	}
