...
final class PluginUpdater {
     ...
     * @var string
	 **/
	private $opt_plugin_info = 'mdp_huger_elementor_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-huger-elementor/huger-elementor.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['huger-elementor/huger-elementor.php'] = (object)[
				'slug' => 'huger-elementor',
                'plugin' => 'huger-elementor/huger-elementor.php',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'huger-elementor' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_huger_elementor_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_huger_elementor_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
            ...
            esc_attr_e( 'Your plugin license is ', 'huger-elementor' );
            echo '<strong style="color: #ff495c">';
            esc_attr_e( 'not activated', 'huger-elementor' );
            echo '.</strong> ';
            esc_attr_e( 'Please activate the license to receive automatic updates in one click, or visit the plugin page on the Envato market to ', 'huger-elementor' );
            echo ' <a href="https://1.envato.market/reviewer-elementor" target="_blank">';
			echo ' <a href="https://1.envato.market/huger-elementor" target="_blank">';
			esc_attr_e( 'download ', 'huger-elementor' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'huger-elementor' );
		}
