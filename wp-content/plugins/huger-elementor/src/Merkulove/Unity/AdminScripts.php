...
final class AdminScripts {
	...
	private function settings_scripts() {
        ...
        wp_enqueue_script( 'mdp-huger-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/merkulov-ui' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );

        /** Add tab related scripts */
        if ( isset( $_GET[ 'tab' ] ) && $_GET[ 'tab' ] === 'migration' ) {
            wp_enqueue_script( 'mdp-huger-elementor-unity-migration-tab', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/migration' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );
        }
        ...
        $to_js = [
            ...
            'nonce'     => wp_create_nonce( 'huger-elementor-unity' ), // Nonce for security.
        ];

        wp_enqueue_script( 'mdp-huger-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
        wp_localize_script( 'mdp-huger-elementor-unity-admin', 'mdpHugerElementorUnity', $to_js );
        wp_enqueue_script( 'mdp-huger-elementor-admin', Plugin::get_url() . 'js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
