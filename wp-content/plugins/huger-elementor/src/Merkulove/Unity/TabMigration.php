...
final class TabMigration extends Tab {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_export_settings_huger_elementor', [ __CLASS__, 'ajax_export_settings' ] );
        add_action( 'wp_ajax_import_settings_huger_elementor', [ __CLASS__, 'ajax_import_settings' ] );
	...
	public function add_settings() {
        ...
        $group = 'HugerElementor' . self::TAB_SLUG . 'OptionsGroup';
        $section = 'mdp_huger_elementor_' . self::TAB_SLUG . '_page_status_section';

        // Add migration controls only for general plugins
        if ( ! in_array( Plugin::get_type(), [ 'elementor', 'wpbakery', 'divi' ] ) ) {
            add_settings_field( 'import_settings', esc_html__( 'Import settings:', 'huger-elementor' ), [$this, 'render_import_settings'], $group, $section );
            add_settings_field( 'export_settings', esc_html__( 'Export settings:', 'huger-elementor' ), [$this, 'render_export_settings'], $group, $section );
        }
		add_settings_field( 'delete_plugin', esc_html__( 'Removal settings:', 'huger-elementor' ), [$this, 'render_delete_plugin'], $group, $section );
	...
	public function render_delete_plugin() {
		...
		$uninstall_settings = get_option( 'mdp_huger_elementor_migration_settings' );
		...
		/** Prepare description. */
		$helper_text = esc_html__( 'Choose which data to delete upon using the "Delete" action in the "Plugins" admin page.', 'huger-elementor' );

		/** Render select. */
		UI::get_instance()->render_select(
			$options,
			$uninstall_settings[ 'delete_plugin' ], // Selected option.
			esc_html__( 'Delete plugin', 'huger-elementor' ),
			$helper_text,
			[
			    'name' => 'mdp_huger_elementor_migration_settings[delete_plugin]'
            ]
		);
    ...
    public function render_import_settings() {
        ...
        UI::get_instance()->render_import(
            '',
            esc_html__( 'Import settings', 'huger-elementor' ),
            esc_html__( 'Drag & Drop file here', 'huger-elementor' ),
            'import_settings',
            'migration'
        );
    ...
    public function render_export_settings() {
        ...
        foreach ( $tabs as $slug => $tab ) {
            ...
            UI::get_instance()->render_switcher(
                $options[ 'export_' . $slug ] ?? 'on',
                $tab['label'] . ' ' . esc_html__( 'Tab', 'huger-elementor' ),
                esc_html__( 'Export all settings from', 'huger-elementor' ) . ' ' .  $tab['label'] . ' ' . esc_html__( 'tab', 'huger-elementor' ),
                [
                    "name" => wp_sprintf(
                        'mdp_huger_elementor_%s_settings[export_%s]',
                        self::TAB_SLUG,
                        $slug
                    )
                ]
        ...
        UI::get_instance()->render_button(
            esc_html__( 'Export settings', 'huger-elementor' ),
            '',
            'file_download',
            [
                "name" => 'mdp_huger_elementor_' . self::TAB_SLUG . '_settings' . "[export_selected]",
                "id" => "mdp-export-btn",
    ...
    public function ajax_export_settings() {
        ...
        check_ajax_referer( 'huger-elementor-unity', 'nonce' );
        ...
        foreach ( $options as $opt => $val ) {
            ...
            if ( strpos( $opt,'export_' ) === 0 && $val === 'on' ) {
                ...
                $tab_options = get_option( 'mdp_huger_elementor_' . $tab . '_settings' );
    ...
    public function ajax_import_settings() {
        ...
        foreach ( $settings as $tab => $set ) {
            ...
            if ( $set === false ) {
                ...
                // Reset to default
                delete_option( 'mdp_huger_elementor_' . $tab . '_settings' );
...
', $setting );
                ...
                // Update settings
                update_option( 'mdp_huger_elementor_' . $tab . '_settings', $set );
    ...
    private function import_errors( $e, $plugin = array() ) {
        ...
        switch ( $e ) {
            ...
            case 1:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'No settings for import', 'huger-elementor' )
                );
                break;
            case 2:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Wrong settings format', 'huger-elementor' )
                );
                break;
            case 3:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for', 'huger-elementor' ) . ' ' . ucwords( $plugin[ 'slug' ] ) . esc_html__( ', but this is', 'huger-elementor' ) . ' ' . Plugin::get_name()
                );
                break;
            case 4:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for version', 'huger-elementor' ) . ' ' . ucwords( $plugin[ 'version' ] ) . ', but the current version is' . ' ' . Plugin::get_version()
                );
                break;
            case 5:
                $message = array(
                    'status' => true,
                    'message' => esc_html__( 'Settings imported', 'huger-elementor' )
                );
                break;
            default:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Unknown error', 'huger-elementor' )
                );
