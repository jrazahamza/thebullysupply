...
var WP_Optimize_Premium = function() {
	...
	var unused_images_tab_container = $('#wp-optimize-nav-tab-wpo_images-unused-contents'),
		...
		unused_images_before_scan_view_trash_btn = $('#wpo_scan_for_unused_images_trash_info > a'),
		...
		images_loaded_count = {},
		images_total_count = {},
		images_per_page = parseInt(wpoptimize.unused_images_per_page),
		...
		images_dimensions_btn = $("#images_dimensions"),
		images_dimensions_save_settings_btn = $("#image_dimensions_save_settings_btn"),
		images_dimensions_ignore_classes = $("#image_dimensions_ignore_classes"),
		images_dimensions_hidden_content = $("#image_dimensions_hidden_content");
	...
	unused_images_view_images_btn.on('click', function() {
		...
		// when trash was opened without unused images scanning.
		if (unused_images_view_images_btn.data('before-scan')) {
			jQuery('.wpo-unused-images-section-loaded').addClass('wpo_hidden');
			jQuery('.wpo-unused-images-section-unloaded').show();
			unused_images_view_images_btn.data('before-scan', false);
		}
	...
	/**
	 * Handle switching to unused images trash without site scanning for unused images.
	 */
	unused_images_before_scan_view_trash_btn.on('click', function() {
		jQuery('.wpo-unused-images-section-unloaded').hide();
		jQuery('.wpo-unused-images-section-loaded').removeClass('wpo_hidden');
		
		// show trash
		unused_images_view_trash_btn.trigger('click');
		// set flag to 'back to unused images list' button to know which UI show
		unused_images_view_images_btn.data('before-scan', true);
	});
	...
	$(document).on(IMAGES_EVENTS.GET_INFO_DONE, function(event, data) {
		...
		if (IMAGES_WORK_MODE.DETECT_BOTH == mode) {
			...
			if (meta && meta[last_scan_images_key]) {
				...
				if (meta.hasOwnProperty('trash_images_count') && meta.trash_images_count > 0) {
					$('#wpo_scan_for_unused_images_trash_info > span').text(meta.trash_images_count_message);
					$('#wpo_scan_for_unused_images_trash_info').css('display', 'block');
				}
			}
	...
	function restore_unused_images_from_trash(selected_images, dom_elements_to_remove) {
		...
		return trash_manager_send_command('restore_images_from_trash', {
			...
			if (resp.error) {
				...
				unused_images_trash_view.load_next_page_if_need();
	...
	function disable_image_optimization_controls(disable) {
		...
		$.each([
			...
			restore_unused_images_from_trash_btn,
			remove_unused_images_from_trash_btn,
			unused_images_view_images_btn,
			unused_images_view_trash_btn
		], function(i, el) {
	...
	function load_unused_images_trash_next_page() {
		...
		var blog_id = get_selected_site(),
			images_loaded = unused_images_trash_view.get_images_count(blog_id),
			offset = unused_images_trash_view.get_visible_images_count(blog_id);
	...
	function show_images_loaded_text(blog_id) {
		if (images_total_count.hasOwnProperty(blog_id) && images_total_count[blog_id] > 0) {
			var text = wpoptimize.x_of_x_images_loaded.replace('%s', images_loaded_count[blog_id]).replace('%s', images_total_count[blog_id]);
			$('#wpo_unused_images_loaded_count').text(text);
		} else {
			$('#wpo_unused_images_loaded_count').text('');
		}
	}
	...
	function update_unused_images_view(data) {
		...
		if (data && data.hasOwnProperty('unused_images')) {
			...
			for (blog_id in data.unused_images) {
				...
				if (data.images_loaded_info.hasOwnProperty(blog_id)) {
					images_total_count[blog_id] = data.images_loaded_info[blog_id].total;
				}
		...
		if (data && 0 === data.files) {
			...
			images_total_count[get_selected_site()] = 0;
	...
	function append_image_to_list(image, base_url, admin_url, blog_id, blog_url) {
		var value = [blog_id, (image.id ? image.id : image.orig_url)].join('_'),
	...
	$('#wpo_add_always_purge_btn').on('click', function() {
		...
		var new_row_html = [
			'<tr><td><input type="hidden" class="cache-settings-array" name="always_purge_post_type" value="',post_type.join(','),'">',selected_post_types.join(', '),'</td><td><input type="hidden" class="cache-settings-array" name="always_purge_post_id" value="',post_id,'">',post_title,'</td><td><a class="wpo-always-purge-delete wpo-delete" href="javascript: ;">',wpoptimize.delete,'</a></td></tr>'
		].join('');
	...
	/**
	 * Saves images dimensions
	 *
	 * @param {string} button_clicked which button click triggered the save event (save or toggle)
	 */
	function save_images_dimensions_settings(button_clicked) {
		var spinner = images_dimensions_save_settings_btn.next('.wpo_spinner'),
			success_icon = spinner.next('.dashicons-yes');
		...
		var data = {
			"images_dimensions": images_dimensions_btn.is(":checked"),
			"ignore_classes": images_dimensions_ignore_classes.val()
		};

		send_command('save_images_dimension_option', data, function(response) {
			spinner.hide();
			$.unblockUI();
			if (response.success) {
				if ('save_button' === button_clicked) {
					success_icon.fadeIn('fast').fadeOut('slow');	
				}
			}
		});
	}

	images_dimensions_btn.on('click', function() {
		images_dimensions_hidden_content.css('display', images_dimensions_btn.is(":checked") ? 'block' : 'none');
		save_images_dimensions_settings('toggle_button');
	});

	images_dimensions_save_settings_btn.on('click', function() {
		save_images_dimensions_settings('save_button');
	});
};
