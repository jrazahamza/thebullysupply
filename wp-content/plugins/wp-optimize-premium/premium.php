...
class WP_Optimize_Premium {
	...
	public function enqueue_scripts_styles($min_or_not_internal, $min_or_not, $enqueue_version) {
		...
		wp_enqueue_script('wp-optimize-premium-js', WPO_PLUGIN_URL . 'js/wpo-premium' . $min_or_not_internal . '.js', array('jquery', 'jquery-ui-dialog', 'wp-optimize-send-command', 'wp-optimize-admin-js', 'wp-optimize-images-view-js', 'jquery-ui-datepicker', 'select2', 'block-ui-js'), $enqueue_version);
	...
	public function admin_page_minify_tabs($tabs) {
		$tabs['analytics'] = __('Analytics', 'wp-optimize');
		return $tabs;
	}
					...
					<ul>
						...
						foreach ($sites as $site) {
							...
							<li>
								...
								<img id="optimization_spinner_<?php echo $dom_id; ?>" class="optimization_spinner display-none" src="<?php echo esc_attr(admin_url('images/spinner-2x.gif')); ?>" alt="...">
								<label for="<?php echo $dom_id; ?>"><?php echo esc_attr($site->domain.$site->path) . (1 == $site->blog_id ? '<em>('.__('The network-wide tables will be optimized with this site', 'wp-optimize').')</em>' : ''); ?></label>								
							</li>
						<?php } ?>
					</ul>
				</div>
