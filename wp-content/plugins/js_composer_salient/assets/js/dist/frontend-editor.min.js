...
window.vc || (window.vc = {}),
      ...
      void 0 === window.vc && (window.vc = {}), window.vc.ShortcodesBuilder = function(models) {
          ...
          save: function(status) {
              ...
              string = this.getContent(), (data = {
                  action: "vc_save"
              }).vc_post_custom_css = window.vc.$custom_css.val();
              if(vc.frame_window.jQuery('body.single-portfolio').length > 0) {
                data.nectar_post_type = 'portfolio';
              } else {
                data.nectar_post_type = 'std';
              }
              data.content = this.wpautop(string), status && (data.post_status = status, $(".vc_button_save_draft").hide(100) && $("#vc_button-update").text(window.i18nLocale.update_all)), window.vc.update_title && (data.post_title = this.getTitle()), this.ajax(data).done(function() {
  ...
  function($) {
      ...
      _.isUndefined(window.vc) && (window.vc = {}), $.ajaxSetup({
          ...
          hide: function(e) {
             /* nectar addition */ 
             jQuery(window).trigger('nectar_wpbakery_template_add');
      ...
      window.vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
          ...
          buildParamsContent: function(data) {
              var $panelHeader, $panelTab, data = $(data),
                  ...
                  /* nectar addition - nectarAfterRenderCbBindThis in cb */
                  nectarAfterRenderCbBindThis = nectarAfterRenderCb.bind(this),
                  cb = function() {
                      !window.wpb_edit_form_loaded && counter < 10 ? (counter++, setTimeout(cb, 100)) : (_self.$content.removeAttr("data-vc-param-initialized"), _self.active_tab_index = 0, _self.tabsInit = !1, _self.panelInit = !1, _self.dependent_elements = {}, _self.requiredParamsInitialized = !1, _self.$content.find("[data-vc-param-initialized]").removeAttr("data-vc-param-initialized"), $panelTab.removeClass("visually-hidden"), _self.$content.find(".vc_ui-wp-spinner").remove(), _self.init(), _self.$content.parent().scrollTop(1).scrollTop(0), _self.$content.removeClass("vc_properties-list-init"), _self.$el.trigger("vcPanel.shown"), _self.trigger("afterRender"), nectarAfterRenderCbBindThis())
                  };
              window.setTimeout(cb, 800);
             
              /* nectar addition */
              function nectarAfterRenderCb() {
                $('[data-vc-shortcode="text-with-icon"] .icon.dropdown').chosen({
          ...
          if (this.panelInit) {
            ...
            jQuery(window).trigger('nectar_wpbakery_el_save');
  ...
  function($) {
      ...
      vc.map = {}, vc.setFrameSize = function(size) {
          ...
          paste: function(e) {
              var builder = new vc.ShortcodesBuilder;
              if (e && e.preventDefault && e.preventDefault(), e && e.stopPropagation && e.stopPropagation(), this.builder && !this.builder.is_build_complete) return !1;
              this.builder = builder, vc.clone_index /= 10, vc.pasteShortcode(this.model, builder)
          },
          ...
          initialize: function() {
              vc.frame_window = vc.$frame.get(0).contentWindow, $(vc.frame_window.document.body).on("paste", this.handleEditorPaste)
          },
          handleEditorPaste: function(evt) {
              var evt = evt.originalEvent.clipboardData.getData("text/plain") || "",
                  builder = new vc.ShortcodesBuilder;
              vc.pasteShortcode(!1, builder, evt)
          },
          ...
          render: function() {
              ...
              // nectar addition - removing wpb-content-wrapper warning
              // $(vc.$frame.get(0).contentWindow.document).find(".wpb-content-wrapper").length || window.vc.showMessage(window.i18nLocale.not_editable_post || "This post can not be edited with WPBakery since it is missing a WordPress default content area.", "error", "30000"), 
              this
          },
  ...
  function($) {
      ...
      window.InlineShortcodeViewContainerWithParent = window.InlineShortcodeViewContainer.extend({
          ...
          copyParent: function(e) {
              e && e.preventDefault && e.preventDefault(), this.parent_view.copy(e)
          },
          pasteParent: function(e) {
              e && e.preventDefault && e.preventDefault(), this.parent_view.paste(e)
          },
  ...
  function($) {
      ...
      window.vc.ttaSectionActivateOnClone = !1, window.InlineShortcodeView_vc_tta_section = window.InlineShortcodeViewContainerWithParent.extend({
          ...
          copy: function(e) {
              vc.ttaSectionActivateOnClone = !0, window.InlineShortcodeView_vc_tta_section.__super__.copy.call(this, e)
          },
          paste: function(e) {
              vc.ttaSectionActivateOnClone = !0, window.InlineShortcodeView_vc_tta_section.__super__.paste.call(this, e)
          },
          addSibling: function(e) {
  ...
  function($) {
      ...
      window.vc.cloneMethod_vc_tab = function(data, model) {
          ...
          buildNav: function() {
            ...
            /*nectar addition*/
            this.$tabs = this.tabsControls(); // this.$tabs seems to be wrong in the original code (looking for wpb_tabs above instead of wpb_tabs_nav)
            this.$tabs.parent().find('> .vc_element[data-tag="tab"]').each(function(key) {
                $("li:eq(" + key + ")", $nav).attr("data-m-id", $(this).data("model-id"))
            });

            // Toggle style.
            if(this.$tabs.parent().is('[data-style="toggle_button"]')) {
                var color = (this.$tabs.parent().is('[data-color-scheme]')) ? this.$tabs.parent().attr('data-color-scheme') : 'accent-color';
                var that = this;
                setTimeout(function(){
                    if ( that.$tabs.find('.toggle-button-inner').length > 0 ) {
                        const toggleButtonInner = that.$tabs.find('.toggle-button-inner')[0];
                        toggleButtonInner.removeAttribute('class');
                        toggleButtonInner.classList.add('toggle-button-inner', 'nectar-color-'+color, 'nectar-bg-'+color);
                    } else {
                        $('<li class="toggle-button"><span class="toggle-button-inner nectar-color-'+color+' nectar-bg-'+color+'"><span class="circle"></span></span></li>')
                        .insertAfter(that.$tabs.find('> li:first-child'));
                    }
                    
                    
                }.bind(this),1000);
            } else {
                this.$tabs.find('.toggle-button').remove();
            }
  ...
  function() {
      ...
      window.InlineShortcodeView_vc_tab = window.InlineShortcodeViewContainerWithParent.extend({
          ...
          render: function() {
               ...
               if(tabsVisible == 0) {
              //   this.$tab.css({ 'opacity' : '1', 'position' : 'relative' ,'display':'block', 'left':'0'}); 
               } else {
                // this.$tab.css({ 'opacity' : '0', 'position' : 'absolute' ,'display':'block', 'left':'0'}); 
               }
          ...
          allowAddControl: function() {
              /* Nectar addition */
              return vc_user_access().shortcodeAll("tab")
              /* Nectar addition end */
          },
