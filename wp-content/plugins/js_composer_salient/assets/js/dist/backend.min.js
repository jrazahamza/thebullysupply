...
window.vc || (window.vc = {}),
        ...
        window.vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
            ...
            show: function() {
                ...
                [].forEach.call(document.querySelectorAll('.vc_ui-template img[data-src]'), function(img) {
                  ...
                  // Create Salient Studio Markup
                  if( $('.vc_ui-template-list.salient-studio-template-json .json-templates').length > 0 ) {
                    
                    var $studioTemplateMarkup = '';
                    var $studioTemplateIMG = '';
                    var $studioTemplateCategory = '';
                    var $studioTemplateButton = '';
                    
                    var $studioTemplatesContainer = $('.vc_ui-template-list.salient-studio-template-json');
                    var $studioTemplatesJSON      = JSON.parse($studioTemplatesContainer.find('.json-templates').text());
                    
                    $studioTemplatesContainer.find('.json-templates').remove();
                    
                    if($studioTemplatesJSON.length > 0) {
                      
                      $.each($studioTemplatesJSON,function(k,v){
  
                        $studioTemplateMarkup = $('<div class="vc_ui-template vc_templates-template-type-'+ v.template_type+ ' ' + v.custom_class +'" data-vc-content=".vc_ui-template-content"></div>');
                        $studioTemplateMarkup.attr('data-template_id',v.template_id);
                        $studioTemplateMarkup.attr('data-template_id_hash',v.template_id_hash);
                        $studioTemplateMarkup.attr('data-category',v.template_type);
                        $studioTemplateMarkup.attr('data-template_unique_id',v.template_id);
                        $studioTemplateMarkup.attr('data-template_name',v.template_name_lower);
                        $studioTemplateMarkup.attr('data-template_type',v.template_type);
                        
                        $studioTemplateMarkup.append('<div class="vc_ui-list-bar-item"></div>');
                        
                        $studioTemplateIMG = '<img src="" data-src="'+v.img+'" alt="'+ v.template_name +'" width="300" height="200" />';
                        $studioTemplateCategory = '<div class="display_cat">'+ v.template_category_name +'</div>';
                        $studioTemplateButton = '<button type="button" class="vc_ui-list-bar-item-trigger" data-template-handler="" data-vc-ui-element="template-title">'+v.template_name+'</button>';
                        
                        $studioTemplateMarkup.find('.vc_ui-list-bar-item').append('<div class="img-wrap"></div>');
                        $studioTemplateMarkup.find('.vc_ui-list-bar-item .img-wrap').append($studioTemplateIMG);
      
                        $studioTemplateMarkup.find('.vc_ui-list-bar-item').append($studioTemplateCategory);
                        $studioTemplateMarkup.find('.vc_ui-list-bar-item').append($studioTemplateButton);
                        
                        $studioTemplatesContainer.append($studioTemplateMarkup);
                      });
                      
                      // Start orderby on date.
                      $('select#salient-studio-sorting').val('date').trigger('change');
  
                      // lazy load images in chunks after sorted.
                      var lazyItems = [].slice.call(document.querySelectorAll('.vc_ui-template img[data-src]'));
  
                      var lazyItemObserver = new IntersectionObserver(function (entries) {
                          entries.forEach(function (entry) {
                              if (entry.isIntersecting) {
                                  var lazyItem = entry.target;
                                  var imgSrc = lazyItem.getAttribute('data-src');
    ...
    function($) {
        ...
        window.vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
            ...
            buildParamsContent: function(data) {
                ...
                function nectarVcAfterRender() {
                    ...
                    function mouseParallaxDepends(){
                        if($(this).is(':checked')) {
                        $('.bg_position, .bg_repeat').parent().parent().hide();
                        ...
                        $('.bg_position, .bg_repeat').parent().parent().show();
            ...
            init: function() {
                ...
                this.$el.find('[data-vc-ui-element="panel-header-content"]').removeClass("visually-hidden"), this.initParams(), this.initDependency();
    ...
    function($) {
        ...
        vc.debug = !1, vc.map = _.isUndefined(window.vc_mapper) ? {} : window.vc_mapper, vc.roles = _.isUndefined(window.vc_roles) ? {} : window.vc_roles, vc.Storage = function() {
          ...
          //portfolio extra
             ...
             content = vc_wpautop(content), $("textarea#_nectar_portfolio_extra_content").val(content), contentTinyMce && contentTinyMce.setContent && (contentTinyMce.setContent(content), contentTinyMce.fire("change"))
           }
    ...
    function($) {
        ...
        vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
            ...
            /* nectar addition - escape tags */
            sanitizeTags: function(str) {
            ...
            changeShortcodeParams: function(model) {
                ...
                _.defer(function() {
                    ...
                    _.isObject(vc.atts[param_settings.type]) && _.isFunction(vc.atts[param_settings.type].render) && (value = vc.atts[param_settings.type].render.call(this, param_settings, value)), $wrapper.children("." + param_settings.param_name).is("input,textarea,select") ? $wrapper.children("[name=" + param_settings.param_name + "]").val(value) : $wrapper.children("." + param_settings.param_name).is("iframe") ? $wrapper.children("[name=" + param_settings.param_name + "]").attr("src", value) : $wrapper.children("." + param_settings.param_name).is("img") ? ($img = $wrapper.children("[name=" + param_settings.param_name + "]"), value && value.match(/^\d+$/) ? $.ajax({
                        ...
                        $img.attr("src", url)
                         // nectar addition -- added sanitizeTags to vc_wpautop and label_value
                    }) : value && $img.attr("src", value)) : $wrapper.children("[name=" + param_settings.param_name + "]").html(value ? vc_wpautop(this.sanitizeTags(value)) : ""), name.length && ("" === value || _.isUndefined(value) ? name.hide().addClass("hidden-label") : (_.isObject(param_settings.value) && !_.isArray(param_settings.value) && "checkbox" === param_settings.type ? (inverted_value = _.invert(param_settings.value), label_value = _.map(value.split(/[\s]*\,[\s]*/), function(val) {
                        return _.isString(inverted_value[val]) ? inverted_value[val] : val
                    }).join(", ")) : _.isObject(param_settings.value) && !_.isArray(param_settings.value) && (inverted_value = _.invert(param_settings.value), label_value = _.isString(inverted_value[value]) ? inverted_value[value] : value), name.html("<label>" + name.find("label").text() + "</label>: " + this.sanitizeTags(label_value)), name.show().removeClass("hidden-label")))  // nectar addition -- added sanitizeTags to label_value
    ...
    function($) {
        ...
        vc.saved_custom_css = !1, vc.createPreLoader = function() {
            ...
            save: function() {
                ...
                var $postStatus = ($('input[name="hidden_post_status"]').length > 0) ? $('input[name="hidden_post_status"]').val() : 'published';
    ...
    function($) {
        ...
        window.VcRowView = vc.shortcode_view.extend({
            ...
            buildDesignHelpers: function() {
              ...
              var bg_color = this.model.getParam("bg_color");
             ...
             // matches = css.match(/background\-image:\s*url\(([^\)]+)\)/), matches && !_.isUndefined(matches[1]) && (image = matches[1]), matches = css.match(/#[A-Fa-f0-9]{6}/), matches && !_.isUndefined(matches[1]) && (color = matches[1]), matches = css.match(/background:\s*([^\s]+)\b\s*url\(([^\)]+)\)/), matches && !_.isUndefined(matches[1]) && (color = matches[1], image = matches[2]), 
              ...
              var rowId = this.model.getParam("id"); 
              this.$el.find("> .vc_controls .vc_row-hash-id").remove();
              if( !_.isEmpty(rowId) ) {
                $('<span class="vc_row-hash-id"></span>').text("#" + rowId).insertAfter($elementToPrepend);
              } 
              /* nectar addition end */ 
            },
            addElement: function(e) {
           ...
           var bg_color = this.model.getParam("background_color");
           ...
           //this.$el.find("> .vc_controls .vc_column_color").remove(), this.$el.find("> .vc_controls .vc_column_image").remove(), matches = css.match(/background\-image:\s*url\(([^\)]+)\)/), matches && !_.isUndefined(matches[1]) && (image = matches[1]), matches = css.match(/background\-color:\s*([^\s\;]+)\b/), matches && !_.isUndefined(matches[1]) && (color = matches[1]), matches = css.match(/background:\s*([^\s]+)\b\s*url\(([^\)]+)\)/), matches && !_.isUndefined(matches[1]) && (color = matches[1], image = matches[2]), image && $('<span class="vc_column_image" style="background-image: url(' + image + ');" title="' + i18nLocale.column_background_image + '"></span>').insertBefore($column_toggle), color && $('<span class="vc_column_color" style="background-color: ' + color + '" title="' + i18nLocale.column_background_color + '"></span>').insertBefore($column_toggle)
            ...
            events: {
                ...
                "click > .vc_controls .column_clone, > .vc_controls .vc_control-btn-clone": "clone",
                 // nectar addition
                 "click > .vc_controls .column_copy, > .vc_controls .vc_control-btn-copy": "copy",
                 "click > .vc_controls .column_paste, > .vc_controls .vc_control-btn-paste": "paste",
                 // nectar addition end
            },
            render: function() {
                // nectar addition. vc_accordion_tab to toggles
                return window.VcAccordionView.__super__.render.call(this), vc_user_access().shortcodeAll("toggles") ? vc_user_access().partAccess("dragndrop") && this.$content.sortable({
                    axis: "y",
                    handle: "h3",
                    stop: function(event, ui) {
            ...
            addTab: function(e) {
                // nectar addition shortcodeAll(vc_accordion_tab) to toggles
                if (e && e.preventDefault && e.preventDefault(), !vc_user_access().shortcodeAll("toggles")) return !1;
            ...
            createAddTabButton: function() {
                ...
                // nectar addition vc_tab to tab
                this.$tabs.append('<div id="new-tab-' + new_tab_button_id + '" class="new_element_button"></div>'), this.$add_button = $('<li class="add_tab_block"><a href="#new-tab-' + new_tab_button_id + '" class="add_tab" title="' + window.i18nLocale.add_tab + '"></a></li>').appendTo(this.$tabs.find(".tabs_controls")), vc_user_access().shortcodeAll("tab") || this.$add_button.hide()
            },
            addTab: function(e) {
                  /* nectar addition */
                  // nectar addition vc_tab to tab
                 if (e && e.preventDefault && e.preventDefault(), !vc_user_access().shortcodeAll("tab")) return !1;
            ...
            changeShortcodeParams: function(model) {
                ...
                _.isArray(tag.params) && _.each(tag.params, function(param_settings) {
                    ...
                    !_.isUndefined(param_settings.admin_label) && param_settings.admin_label && (param_settings = param_settings.param_name, value = params[param_settings], ($admin_label = this.$el.find("> .wpb_element_wrapper").children(".admin_label_" + param_settings)).length) && ("" === value || _.isUndefined(value) ? $admin_label.hide().addClass("hidden-label") : ("type" !== param_settings || _.isUndefined(params["icon_" + value]) || (value = vc_toTitleCase(value) + " - <i class='" + params["icon_" + value] + "'></i>"), $admin_label.html("<label>" + $admin_label.find("label").text() + "</label>: " + this.sanitizeTags(value)), $admin_label.show().removeClass("hidden-label"))) // nectar addition sanitizeTags added to value
