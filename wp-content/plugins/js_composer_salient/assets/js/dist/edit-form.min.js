...
function($) {
  ...
  media = wp.media, preloader_url = ajaxurl.replace(/admin\-ajax\.php/, "images/wpspin_light.gif"), media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
      ...
      parse: function(value) {
          ...
          value && value[2] && this.parseAtts(value[2].replace(/\s+!important/g, ""))
      },
      ...
      setBgImage: function() {
          ! function(selection, obj) {
              ...
              ids = [], $(".media-modal").addClass("processing-media"), selection.each(function(model) {
                  ...
                  }(ids, function(newAttachments) {
                      if (!window.vc || !window.vc.active_panel) return !1;
                      var newAttachments = _.map(newAttachments, function(newAttachment) {
                              return newAttachment.attributes
                          }),
                          newAttachments = (selection.reset(newAttachments), _.map(selection.models, function(model) {
                              return model.attributes
                          })),
                          template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {
                              variable: "img"
                          }, vc.templateOptions.custom));
                      obj._css_editor.$el.find(".vc_background-image .vc_image").html(template(newAttachments[0])), $(".media-modal").removeClass("processing-media")
                  })
          }(this.get("selection"), this)
      },
      setCurrentBgImage: function(value) {
          var url, template, image_regexp = /([^\?]+)(\?id=\d+){0,1}/,
              id = "";
          ...
          })) : value.match(image_regexp) && (url = (image_regexp = value.split(image_regexp))[1], image_regexp[2] && (id = image_regexp[2].replace(/[^\d]+/, "")), template = vc.template($("#vc_css-editor-image-block").html(), _.defaults({}, {
              variable: "img"
          }, vc.templateOptions.custom)), this.$el.find(".vc_background-image .vc_image").html(template({
              url: url,
              id: id
      ...
      save: function() {
          ...
          return this.attrs = {}, _.each(this.layouts, function(type) {
              this.getFields(type)
          }, this), this.getBackground(), this.getBorder(), (string = _.isEmpty(this.attrs) ? string : ".vc_custom_" + Date.now() + "{" + _.reduce(this.attrs, function(memo, value, key) {
              return value ? memo + key + ": " + value + " !important;" : memo
          }, "", this) + "}") && vc.frame_window && vc.frame_window.vc_iframe.setCustomShortcodeCss(string), string
      },
      ...
      getFields: function(type) {
          ...
          _.each(this.positions, function(pos) {
              ...
              (val = val.match(/^-?\d*(\.\d+){0,1}(%|in|cm|mm|em|rem|ex|pt|pc|px|vw|vh|vmin|vmax)$/) ? val : isNaN(parseFloat(val)) ? "" : parseFloat(val) + "px") && val.length && data.push({
                  name: pos,
                  val: val
              })
      ...
      getSimplifiedField: function(type) {
          ...
          (val = val.match(/^-?\d*(\.\d+){0,1}(%|in|cm|mm|em|rem|ex|pt|pc|px|vw|vh|vmin|vmax)$/) ? val : isNaN(parseFloat(val)) ? "" : parseFloat(val) + "px").length && (this.attrs[type] = val)
      }
  }), vc.atts.css_editor = {
      parse: function(param) {
      ...
      init: function(param, $field) {
          $("[data-css-editor=true]", this.content()).each(function() {
              ...
              }, params = vc.edit_element_block_view.model.get("params"), value = (keys = _.reduce(keys, function(memo, css_name, attr_name) {
              ...
              }, "")) ? ".tmp_class{" + keys + "}" : ""), $param.data("vcFieldManager", new VcCssEditor({
                  el: $editor
              }).render(value))
...
function($) {
  ...
  var ParamsPresetView = Backbone.View.extend({
      ...
      setParams: function() {
          var fields, val = this.save(),
              val = this.$el.find('[value="' + val + '"]'),
              $edit_form = vc.edit_element_block_view.content(),
              $params_preset = this.$el;
          val.length && (val = val.data("params"), fields = [], _.each(val, function(value, key, list) {
              var fieldManager, key = $edit_form.find("[name=" + key + "].wpb_vc_param_value");
              key.length && (key.unbind("change.vcParamsPreset"), (fieldManager = key.data("vcFieldManager")) && fieldManager.render && fieldManager.render(value), fields.push(key), key.val(value).trigger("change"))
...
function($) {
  "use strict";
  new function() {
      $(".gallery_widget_attached_images_list", this.$view).off("click.removeImage").on("click.removeImage", "a.vc_icon-remove", function(e) {
          e.preventDefault();
          var e = $(this).closest(".edit_form_line"),
              img_ids = ($(this).parent().remove(), []);
          e.find(".added img").each(function() {
              img_ids.push($(this).attr("rel"))
          }), e.find(".gallery_widget_attached_images_ids").val(img_ids.join(",")).trigger("change")
...
function($) {
  ...
  vc.atts.vc_grid_item = {
      ...
      parse: function(param) {
          param = this.content().find('[data-vc-shortcode-param-name="' + param.param_name + '"] [data-vc-grid-element="value"]');
          return param.length ? param.val() : ""
      }
  }
}(window.jQuery);